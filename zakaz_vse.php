...
$result = mysql_query($qwer) or die("Query failed : " . mysql_error());
				...
				print  strlen($line[firm])>0 ? "<OPTION VALUE='$line[id_klient]'> $line[firm]</option>":"";
...
and (Z.id_klient=Q.id_klient)) order by Q.data,Q.id_zakaz"; 
...
$line[id_zakaz]="<b>$id_zakaz</b><hr><img src='images/1.gif' alt='Безналичная оплата'>";
	...
	else {print  $data_user['is_turist']==0 ? "<td>":"<td bgcolor='#FFC0CB'>";}

$query2 = "SELECT id_klient FROM zakaz where id_zakaz='$id_zakaz'"; 
$result2 = mysql_query($query2) or die("Query failed : " . mysql_error());
$line2 = mysql_fetch_array($result2, MYSQL_ASSOC);

echo"<form name='formasum' action='restoim.php' method='get'>
...
echo"<form onsubmit='return confirmAction(event);' name='forma' action='podtzakaz.php' method='get'>
	<input type=hidden value='$id_zakaz' name='id_zakaz'>
	<input type=hidden value='$line2[id_klient]' name='id_klient'>";


echo'<input type=submit value="Подтвердить" onClick="return window.confirm(\'Подтвердить?\')"></form>';


 echo"  <form onsubmit='return confirmAction(event);' name='forma2' action='delzakaz.php' method='get'>
	<input type=hidden value='$id_zakaz' name=id_zakaz>
	<input type=hidden value='$line2[id_klient]' name='id_klient'>"; 

echo'<input type=submit value="Отказать" onClick="return window.confirm(\'Отказать?\')"></form>';

if($line[mesto]<>'без мест')
{
echo"<form action='turist1.php'>
<input type=hidden value='$id_zakaz' name='id_zakaz'>
	<input type=hidden value='$line2[id_klient]' name='id_klient'>
	...
	<input type=hidden value='zakaz' name=perenapr>
<input type=submit value='Подробнее'></form>";
}
else 
{
echo"<form action='turist_besmest1.php'>
<input type=hidden value='$id_zakaz' name='id_zakaz'>
	<input type=hidden value='$line2[id_klient]' name='id_klient'>
	<input type=hidden value='$line[firm]' name=firm>
	<input type=hidden value='$line[city]' name=city>
