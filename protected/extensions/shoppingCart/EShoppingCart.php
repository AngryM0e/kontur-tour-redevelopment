...
class EShoppingCart extends CMap {
    ...
    public function put(IECartPosition $position, $quantity = 1) {
        ...
        if ($this->itemAt($key) instanceof IECartPosition) {
            ...
            //надо сделать перед присоединение нового поведения потому что оно
            //занулит востановленные из сессии значения
            $oldUoptions = $position->getUoptions();
        }

        $this->update($position, $quantity,$oldUoptions);
     ...
     * @param int $quantity
     */
    public function update(IECartPosition $position, $quantity, $oldUoptions) {
        ...
        if($this->_uoptions !== 'undefinded'){
            $oldUoptions[$quantity]=$this->_uoptions;
            $position->setUoptions($oldUoptions);
        }
    ...
    public function deletePositionItem(IECartPosition $position, $quantity) {
        if (!($position instanceof CComponent))
            throw new InvalidArgumentException('invalid argument 1, product must implement CComponent interface');
        ...
        $position->setQuantity($quantity);
        $position->setUoptions($this->_uoptions);
    ...
    public function deletePositionItemNoOpt(IECartPosition $position, $quantity) {
        if (!($position instanceof CComponent))
            throw new InvalidArgumentException('invalid argument 1, product must implement CComponent interface');
