...
class AzovseaPagesController extends Controller
{
    ...
    public function actionPage($url) {
        $this->layout = false;
        $content = AzovseaPages::model()->findByAttributes(array("url" => $url));
...
/*    public function actionIndex() {
        ...
        $main_page_settings= MainAzovseaPagesettings::model()->findByPk(1);
    ...
    public function actionCreate() {
        if (Yii::app()->user->id == 1) {
            $model = new AzovseaPages;

            // Uncomment the following line if AJAX validation is needed
            // $this->performAjaxValidation($model);

            if (isset($_POST['AzovseaPages'])) {
                $model->attributes = $_POST['AzovseaPages'];
                $model->disp = $_POST['AzovseaPages']['disp'];
                $model->sort_num = $_POST['AzovseaPages']['sort_num'];
                if ($model->save())
    ...
    public function actionAdmin() {
        if (Yii::app()->user->id == 1) {
            $model = new AzovseaPages('search');
            $model->unsetAttributes();  // clear any default values
            if (isset($_GET['AzovseaPages']))
                $model->attributes = $_GET['AzovseaPages'];
    ...
    public function loadModel($id) {
        $model = AzovseaPages::model()->findByPk($id);
        if ($model === null)
    ...
    protected function performAjaxValidation($model) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'AzovseaPages-form') {
