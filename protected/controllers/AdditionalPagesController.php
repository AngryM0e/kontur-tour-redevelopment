...
class AdditionalPagesController extends Controller
{
    ...
    public function actionPage($url) {
        $content = AdditionalPages::model()->findByAttributes(array("url" => $url));
...
/*    public function actionIndex() {
        ...
        $main_page_settings= MainAdditionalPagesettings::model()->findByPk(1);
    ...
    public function actionCreate() {
        if (Yii::app()->user->id == 1) {
            $model = new AdditionalPages;

            // Uncomment the following line if AJAX validation is needed
            // $this->performAjaxValidation($model);

            if (isset($_POST['AdditionalPages'])) {
                $model->attributes = $_POST['AdditionalPages'];
                $model->disp = $_POST['AdditionalPages']['disp'];
                $model->sort_num = $_POST['AdditionalPages']['sort_num'];
                if ($model->save())
    ...
    public function actionAdmin() {
        if (Yii::app()->user->id == 1) {
            $model = new AdditionalPages('search');
            $model->unsetAttributes();  // clear any default values
            if (isset($_GET['AdditionalPages']))
                $model->attributes = $_GET['AdditionalPages'];
    ...
    public function loadModel($id) {
        $model = AdditionalPages::model()->findByPk($id);
        if ($model === null)
    ...
    protected function performAjaxValidation($model) {
        if (isset($_POST['ajax']) && $_POST['ajax'] === 'AdditionalPages-form') {
